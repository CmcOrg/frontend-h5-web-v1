...
export interface SignEmailForgotPasswordDTO {
    code: string // 邮箱验证码 {"regexp":"^[0-9]{6}$"}
    newPassword: string // 前端加密之后的新密码
    origNewPassword: string // 前端加密之后的原始新密码
    email: string // 邮箱 {"sizeMax":200,"sizeMin":0,"regexp":"^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$"}
}

// 忘记密码
export function SignEmailForgotPassword(form: SignEmailForgotPasswordDTO, config?: AxiosRequestConfig) {
    return $http.myPost<string>('/sign/email/forgotPassword', form, config)
}

// 忘记密码-发送验证码
export function SignEmailForgotPasswordSendCode(form: EmailNotBlankDTO, config?: AxiosRequestConfig) {
    return $http.myPost<string>('/sign/email/forgotPassword/sendCode', form, config)
}
